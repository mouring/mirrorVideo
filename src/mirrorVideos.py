...
class googleAPI:
    ...
    # q={UserName}
    def search(self, url):
        self.url = "https://www.googleapis.com/youtube/v3/search?part=snippet&" + url + "&key=" + self.apiKey
...
class ytChannelMirror:
  ...
  # fetchFormat = 'bestvideo[ext=mp4]+bestaudio[ext=m4a]'
    fetchFormat = "bestvideo+bestaudio[ext=m4a]/best"
    fetchDir = "/opt/yt/download"
    fetchFile = "%(uploader)s-%(title)s.%(ext)s"
    ...
    def __init__(self, mapName, apiKey):
        ...
        if "items" in data:
            self.listId = data["items"][0]["contentDetails"]["relatedPlaylists"][
                "uploads"
            ]
            ...
            if "items" in data:
                ...
                print(data)
                print("-----")
                self.gAPI.search("q=" + self.listId +"&type=channel")
                data = self.gAPI.jsondata()
